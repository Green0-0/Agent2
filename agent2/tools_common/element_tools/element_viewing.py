...
def view_element(state: AgentState, settings: ToolSettings, path: str, identifier: str):
    ...
    if len(element.elements) > 0:
        subelements = ", ".join(e.identifier for e in element.elements)
...
def semantic_search_elements(state: AgentState, settings: ToolSettings, query: str, 
    ...
    if not files:
        return f"No files found matching path '{path}' and extensions '{extensions}'"
    ...
    if not matches:
        return f"No elements semantically matching '{query}' found"
