    ...
    def step(self, response_str : str):
        ...
        if isinstance(self.tool_formatter, CodeACTToolFormatter):
            try:
                ...
                executable_tools = {}
                for tool in self.tools_list:
                    # The executable tool should be the original tool, but with the first param set to all_files
                    # Therefor, create a partial of the tool
                    tt = functools.partial(tool, self.cached_state, self.tools_settings)
                    executable_tools[tool.name] = tt
                ...
                if "def" in tool_call or "class" in tool_call:
                    error_output += "\nPlease do not use tool call code blocks for anything besides using tools. If you want to modify a piece of code, output a code block and then use replace_element, which will automatically reference the previous code block."
        ...
        try:
            ...
            print("==== TOOL RESPONSE: ====")
            print(tool_response)
            response_true = self.perform_substitutions(self.tool_response_wrapper, tool_name=tool_call["name"], tool_response=tool_response)
