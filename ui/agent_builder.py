...
from agent2.tools_common.basic_tools.basic_viewing import view_lines, search_files, open_file
...
def agent_builder(available_tools: list):
    ...
    settings['max_search_result_listings'] = cols[0].number_input(
        "Max Search Results", 1, 100, settings['max_search_result_listings']
    )
    settings['max_search_result_lines'] = cols[1].number_input(
        "Max Search Lines", 1, 500, settings['max_search_result_lines']
    )
    settings['max_view_lines_start'] = cols[0].number_input(
        "Max View Lines Start", 1, 500, settings['max_view_lines_start']
    )
    settings['max_view_lines_end'] = cols[1].number_input(
        "Max View Lines End", 1, 500, settings['max_view_lines_end']
    )
    ...
    if st.button("Save Agent Configuration"):
        try:
            ...
            formatter_map = {
                ...
                "XML": XMLToolFormatter(),
                "Markdown": MarkdownToolFormatter(),
                "CodeAct": CodeACTToolFormatter()
            }
            ...
            # Save using the updated utility function
